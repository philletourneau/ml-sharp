# PyInstaller spec for a Windows-only self-contained GPU mesh/USDZ GUI build.
#
# Expected build input:
# - a built gsplat CUDA extension copied to `packaging/windows/assets/gsplat_cuda.pyd`
#
# Build:
#   pyinstaller --clean --noconfirm packaging/windows/sharp-usdz-gui.spec
...
a = Analysis(
    [str(ROOT / "packaging" / "windows" / "sharp_usdz_gui_entry.py")],
    pathex=[str(ROOT)],
    binaries=binaries,
    datas=datas,
